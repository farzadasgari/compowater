...
def test_download_writes_content_and_hash_matches(tmp_path, requests_mock):
    ...
    path, digest = download_file(dataset_name="Test", source_page="https://example.gov",
...
def test_existing_file_is_skipped_by_default(tmp_path, requests_mock):
    ...
    download_file(dataset_name="Test",
                  source_page="https://example.gov", url=URL, destination=dest)
...
def test_overwrite_true_redownloads_existing_file(tmp_path, requests_mock):
    ...
    download_file(dataset_name="Test", source_page="https://example.gov",
                  url=URL, destination=dest, overwrite=True)
...
def test_checksum_mismatch_raises_and_leaves_no_file_behind(tmp_path, requests_mock):
    ...
    with pytest.raises(ValueError, match="Checksum mismatch"):
        download_file(dataset_name="Test", source_page="https://example.gov",
                      url=URL, destination=dest, expected_sha256="0" * 64)
...
def test_interrupted_download_cleans_up_partial_file(tmp_path, requests_mock, monkeypatch):
    requests_mock.get(URL, content=CONTENT)
    dest = tmp_path / "data.csv"
...
def test_provided_session_is_reused_instead_of_creating_a_new_one(tmp_path, requests_mock, monkeypatch):
    requests_mock.get(URL, content=CONTENT)
    dest = tmp_path / "data.csv"
    ...
    monkeypatch.setattr(
        "compowater.download.http.create_session", fake_create_session)

    my_session = create_session()
    download_file(dataset_name="Test", source_page="https://example.gov",
                  url=URL, destination=dest, session=my_session)
